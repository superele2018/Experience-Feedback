...
rb = load_workbook("input_update.xlsx")
rs = rb['Sheet1']

wanoModel=WanoCode.WanoCode('code.xlsx')
rs_rows_len = len(list(rs.rows))
rs_columns_len = len(list(rs.columns))

filepath='D:\Anaconda\workspace\ef\\紧急停堆'

rb_p=load_workbook('PlantInfo.xlsx')
rs_p=rb_p['sheet1']
...
def init(plantlist,list_RULE,column_selected):
    for plant in plantlist:
        ...
        dic_statics={}
        report_rules={}
        WANO_Distribution= {'WANO_DIRECT_CAUSE': {'whole':{}},
                            'WANO_ROOT_CAUSE': {'whole':{}},
                            'WANO_SYSTEM': {'whole':{}},
                            'WANO_EQUIPMENT': {'whole':{}},
                            }
...
def myXLWR_n(plant, input_row,column_selected,dict_trash_wb=dict_trash_wb):  # write single row to the purpose wb and update the dic_statics
    ...
    # if event_begin <=datetime.datetime.strptime(PlantInfo[plant][2],"%Y-%m-%d"):  # period screening
        if input_row[Tiltle2index('FLAG')].value == '80'\
                and input_row[Tiltle2index('BEFORE_POWER')].value\
                and int(input_row[Tiltle2index('BEFORE_POWER')].value)>0\
                and input_row[Tiltle2index('AFTER_POWER')].value\
                and int(input_row[Tiltle2index('AFTER_POWER')].value)==0:
            if event_begin>datetime.datetime.strptime('1999-12-31',"%Y-%m-%d"): #and event_begin <=datetime.datetime.strptime(PlantInfo[plant][2],"%Y-%m-%d"):#period screening
                for j in range(0, nr):
                    data.append(input_row[j].value)
                report_rule = input_row[Tiltle2index('REPORT_RULE')].value
                report_year = input_row[Tiltle2index('REPORT_YEAR')].value
                WANO_data = {'WANO_ROOT_CAUSE': str(input_row[Tiltle2index('WANO_ROOT_CAUSE')].value),
                             'WANO_DIRECT_CAUSE': str(input_row[Tiltle2index('WANO_DIRECT_CAUSE')].value),
                             'WANO_SYSTEM': str(input_row[Tiltle2index('WANO_SYSTEM')].value),
                             'WANO_EQUIPMENT': str(input_row[Tiltle2index('WANO_EQUIPMENT')].value)
                             }
...
if not test:
    ...
    for i in range(1,rs_rows_len):
        ...
        plant=row[Tiltle2index('UNIFORM_UNIT')].value
